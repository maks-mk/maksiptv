...
**[Посетите наш веб-сайт](https://maks-mk.github.io/maksiptv/)**
...
* Создание скриншотов при просмотре
...
Плеер поддерживает стандартный формат M3U плейлистов. Для корректного разбиения по категориям, каналы должны содержать атрибут `group-title`:

```
#EXTINF:-1 tvg-id="example" group-title="Фильмы",Название канала
...
5. Если количество каналов кажется некорректным — проверьте, не скрыты ли некоторые каналы (отображается и общее, и видимое количество)
6. При проблемах с воспроизведением проверьте логи в папке `logs` 